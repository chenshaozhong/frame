...
class Http {
	 ...
	 * @return string
	 */
	static public function get($url,$timeout=10,$header="")
    {
	 ...
	 * @return string
	 */
	static public function post($url, $post_data=array(), $timeout=10,$header="" , $fields = false)
    {
		if(empty($url)||empty($timeout))
			return false;
		if(!preg_match('/^(http|https)/is',$url))
		...
		switch($code)
		{
			case 1:return self::curlPost($url,$post_data,$timeout,$header , $fields);break;
	 ...
	 * @return string
	 */
	static public function curlPost($url, $post_data=array(), $timeout=5,$header="" , $fields = false)
    {
