...
class FileCacheDriver implements CacheInterface{
    ...
    public function get($key , $realName = false) {
		$content = @file_get_contents( $this->_getFilePath($key , $realName));
    ...
    public function set($key, $value, $expire = 1800 , $realName = false) {
        $value = serialize($value);
		$md5Sign = md5($value);
		$expire = time() + $expire;		
        $content    = '<?php exit;?>' . sprintf('%012d', $expire) . $md5Sign . $value;		
       
	   return @file_put_contents($this->_getFilePath($key, $realName), $content, LOCK_EX);
    }
	...
	private function _getFilePath($key, $realName = false) {

		$key = $realName ? $key : md5($key);
